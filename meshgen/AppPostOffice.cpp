...
#include "../src/main/CrashHandler.h"
...
namespace mq {
	...
	struct AppPostOffice::ClientIdentification
	{
		uint32_t pid;
		std::string account;
		std::string server;
	...
	mq::AppPostOffice::PipeEventsHandler::PipeEventsHandler(AppPostOffice* postOffice) : m_postOffice(postOffice){}

	void mq::AppPostOffice::PipeEventsHandler::OnIncomingMessage(PipeMessagePtr&& message)
	{
	...
	void mq::AppPostOffice::PipeEventsHandler::OnClientConnected() 
	{
	...
	AppPostOffice::AppPostOffice()
		: m_pipeClient{ mq::MQ2_PIPE_SERVER_PATH }
		, m_launcherProcessID(0)
	{
		m_clientDropbox = RegisterAddress("pipe_client2",
	...
	std::unordered_map<const std::string, std::unique_ptr<postoffice::Mailbox>>::iterator AppPostOffice::FindMailbox(
	...
	void AppPostOffice::RouteMessage(PipeMessagePtr&& message, const PipeMessageResponseCb& callback)
	{
	...
	void AppPostOffice::ProcessPipeClient()
	{
		m_pipeClient.Process();
		Process(1000); // make this large just to prevent overflows
	}

	void AppPostOffice::NotifyIsForegroundWindow(bool isForeground)
	{
	...
	void AppPostOffice::RequestActivateWindow(HWND hWnd, bool sendMessage)
	{
		if (IsIconic(hWnd))
		{
	...
	void AppPostOffice::SetGameStatePostOffice(DWORD GameState)
	{
	...
	void mq::AppPostOffice::StopPipeClient()
	{
		static_cast<AppPostOffice&>(GetPostOffice()).m_pipeClient.Stop();
	}


	void mq::AppPostOffice::RoutingFailed(
	...
	{
		static AppPostOffice s_postOffice;
		return s_postOffice;
	}
	...
	namespace pipeclient {
		...
		{
			static_cast<AppPostOffice&>(GetPostOffice()).NotifyIsForegroundWindow(isForeground);
		}

		void RequestActivateWindow(HWND hWnd, bool sendMessage)
		{
			static_cast<AppPostOffice&>(GetPostOffice()).RequestActivateWindow(hWnd, sendMessage);
		}

		void InitializePostOffice()
		{
			static_cast<AppPostOffice&>(GetPostOffice()).Initialize();
		}

		void ShutdownPostOffice()
		{
			static_cast<AppPostOffice&>(GetPostOffice()).Shutdown();
		}

		void PulsePostOffice()
		{
			static_cast<AppPostOffice&>(GetPostOffice()).ProcessPipeClient();
		}

		void SetGameStatePostOffice(DWORD GameState)
		{
			static_cast<AppPostOffice&>(GetPostOffice()).SetGameStatePostOffice(GameState);
		}
